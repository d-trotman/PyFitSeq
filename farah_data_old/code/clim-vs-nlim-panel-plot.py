...
def load_and_process_data(clim_file, nlim_file):
    """
    ...
    combined_data : pandas.DataFrame
        DataFrame containing Clim_Fitness and Nlim_Fitness values
    """
    # Read the CSV files
    try:
    ...
    min_rows = min(len(clim_data), len(nlim_data))
    if len(clim_data) != len(nlim_data):
        print(f"  Warning: Datasets have different numbers of rows. Using first {min_rows} rows.")
    
    # Make sure we use only the rows that exist in both datasets
...
def create_fitness_comparison_plot(combined_data, ax, title, display_limit=None):
    """
    Create a Clim vs Nlim fitness plot on the given axis.
    
    Parameters:
    -----------
    combined_data : pandas.DataFrame
        DataFrame containing Clim_Fitness and Nlim_Fitness columns
    ax : matplotlib.axes.Axes
        The axis to plot on
    title : str
        Title for the plot
    display_limit : float or None
        The limit for displaying the axes (±display_limit). If None, autoscale to show all points.
    ...
    """
    # Calculate correlation using ALL data points (no filtering)
    correlation, p_value = pearsonr(combined_data['Clim_Fitness'], combined_data['Nlim_Fitness'])
    
    # Calculate regression using ALL data points
    if len(combined_data) > 1:
        z = np.polyfit(combined_data['Nlim_Fitness'], combined_data['Clim_Fitness'], 1)
    ...
    scatter = ax.scatter(
        combined_data['Nlim_Fitness'],
        combined_data['Clim_Fitness'],
        alpha=0.6,
    ...
    xmin, xmax = combined_data['Nlim_Fitness'].min(), combined_data['Nlim_Fitness'].max()
    ymin, ymax = combined_data['Clim_Fitness'].min(), combined_data['Clim_Fitness'].max()
    
    # If no display limit is specified, autoscale to show all data
    if display_limit is None:
        # Find the maximum absolute value in both dimensions
        max_abs_x = max(abs(xmin), abs(xmax))
        max_abs_y = max(abs(ymin), abs(ymax))
        
        # Use the larger of the two with a 10% padding
        display_limit = max(max_abs_x, max_abs_y) * 1.1
    ...
    # Customize the plot
    ax.set_xlabel('Nlim Fitness', fontsize=10)
    ax.set_ylabel('Clim Fitness', fontsize=10)
    ...
    q1 = ((combined_data['Nlim_Fitness'] > 0) & (combined_data['Clim_Fitness'] > 0)).mean() * 100
    q2 = ((combined_data['Nlim_Fitness'] < 0) & (combined_data['Clim_Fitness'] > 0)).mean() * 100
    q3 = ((combined_data['Nlim_Fitness'] < 0) & (combined_data['Clim_Fitness'] < 0)).mean() * 100
    q4 = ((combined_data['Nlim_Fitness'] > 0) & (combined_data['Clim_Fitness'] < 0)).mean() * 100
    ...
    pos_y = (ymax - 0) / 2  # Midpoint of positive y
    neg_y = (0 - ymin) / 2  # Midpoint of negative y
    
    # Adjust position to be visible
    pos_x = min(pos_x, xmax * 0.8)
    neg_x = max(neg_x * -1, xmin * 0.8)
    pos_y = min(pos_y, ymax * 0.8)
    neg_y = max(neg_y * -1, ymin * 0.8)
...
def calculate_average_data(data_by_rep):
    ...
    for idx in range(max_idx):
        ...
        for rep, df in data_by_rep.items():
            if idx < len(df):
                for col in ['Clim_Fitness', 'Nlim_Fitness']:
...
def calculate_replicate_correlations(data_by_rep):
    ...
    for rep1, rep2 in rep_pairs:
        ...
        valid_indices = ~(common_data1[['Clim_Fitness', 'Nlim_Fitness']].isna().any(axis=1) | 
                         common_data2[['Clim_Fitness', 'Nlim_Fitness']].isna().any(axis=1))
        
        common_data1 = common_data1[valid_indices]
        common_data2 = common_data2[valid_indices]
        
        if len(common_data1) > 1:
            # Calculate correlations for clim and nlim fitness
            clim_corr, clim_p = pearsonr(
                common_data1['Clim_Fitness'], 
                common_data2['Clim_Fitness']
            )
            nlim_corr, nlim_p = pearsonr(
                common_data1['Nlim_Fitness'], 
                common_data2['Nlim_Fitness']
            )
...
def format_metrics_text(metrics, rep_metrics, cv_metrics, quadrant_data):
    ...
    metrics_text = "Individual Replicate Metrics (Clim vs Nlim):\n"
    ...
    metrics_text += "\nQuadrant Meaning:\n"
    metrics_text += "  Q1 (top-right): Beneficial in both conditions\n"
    metrics_text += "  Q2 (top-left): Beneficial in Clim, deleterious in Nlim\n"
    metrics_text += "  Q3 (bottom-left): Deleterious in both conditions\n"
    metrics_text += "  Q4 (bottom-right): Beneficial in Nlim, deleterious in Clim\n"
    ...
    for pair, values in rep_metrics.items():
        ...
        if not np.isnan(values['clim_corr']):
            metrics_text += f"  {rep1_name} vs {rep2_name} (n={values['n_common']}):\n"
            metrics_text += f"    Clim fitness: r={values['clim_corr']:.3f} (p={values['clim_p']:.2e})\n"
            metrics_text += f"    Nlim fitness: r={values['nlim_corr']:.3f} (p={values['nlim_p']:.2e})\n"
        else:
    ...
    for metric, cv in cv_metrics.items():
        if metric.endswith('_cv') and not metric.startswith('q'):
...
def plot_all_replicates_plus_average(base_path, output_file, display_limit=None):
    """
    Create a panel plot with Clim Fitness vs Nlim Fitness for all three replicates
    ...
    for rep, files in file_paths.items():
        print(f"\nProcessing {rep}:")
        data = load_and_process_data(files['clim'], files['nlim'])
    ...
    for i, rep in enumerate(['rep1', 'rep2']):
        ...
        correlation, p_value, slope, intercept, quad_percentages = create_fitness_comparison_plot(
    ...
    fig.suptitle('Fitness Comparison: Clim vs Nlim Across Replicates', fontsize=16, y=0.98)
...
if __name__ == "__main__":
    ...
    output_file = os.path.join(output_dir, "clim_vs_nlim_panel_plot.png")
    ...
    # Generate panel plot
    print("Generating Clim vs Nlim fitness panel plot (autoscaling to show all data)...")
